...
public class StreamingSplit {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // CNY -> USD 汇率流
        SingleOutputStreamOperator<ExchangeRateInfo> cnyToUsd =
                env.addSource(new ExchangeRateDataSource(CurrencyType.CNY, CurrencyType.USD, 7, 6), "CNY-USD")
        ...
        DataStream<String> map = cnyToUsd.connect(orderDs)
                .map(new CoMapFunction<ExchangeRateInfo, OrderInfo, String>() {
                    @Override
                    public String map1(ExchangeRateInfo value) {
                        return value.toString();
                    }

                    @Override
                    public String map2(OrderInfo value) {
                        return value.toString();
                    }
                });
